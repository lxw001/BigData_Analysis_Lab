...
public class ProbabilityTable {
	...
	private static double[][][] conditionalProbabilityTable = new double[20][2][2];//条件概率（statics/sum(type[k]）
	private static double[][]   typeProbabilityTable = new double[20][2];//（statics/total）
	private static double[]     classProbability = new double[2];//（statics/total）
	...
	public ProbabilityTable(String pathstr) throws IOException {
		...
		while(lineReader.readLine(line) > 0) {
			...
			if(items.length == 4) {
				//一行有4个数据。如 ：0,0,1,353783	指第一个属性 > 0且属于type0的数据总数数为68711
			...
			} else {//一行有3个数据，如：	type,0,450126	指属于type0的数据总共有450126个（行）
